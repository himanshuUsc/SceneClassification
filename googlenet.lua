...
--    print("inside load loadWeights")
   
--    local function loadWeight(m)
--      -- print("again deep inside in loadweight")
     
--       if m.gname then
--           count=count+1
--           --print(count)
--         --print("m.gname is ", m.gname)
...
local function googlenet(lib)
   ...
   --model:add(nn.Narrow(2, 2, 1000))
   m.gname = 'softmax2'
   model:add(m)
   -- -- because google has 1008 classes, class 2 - 1001 are valid
   ...
   --print("total times loadweight  got called is.."..count)
